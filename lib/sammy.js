...
(function($, window) {
  ...
  (function(){
    ...
    Sammy.Component = Sammy.Object.extend({
      init: function(component_function, name, app, parentComponent){
        ...
        // @TODO decide whether context_prototype should be per app or per component. Probably per component
    ...
    // @TODO take into account parent namespaces
    ...
    Sammy.Component.prototype.removeComponent = function(name) {
      if(this.components[name]) {
        var listenersToRemove = [],
            component = this;
        $.each(this.app._listeners, function(namespacedEvent) {
          if(namespacedEvent.indexOf(component.components[name].eventNamespace()) == 0 ) {
            listenersToRemove.push(namespacedEvent);
          }
        });

        for(var i=0, cnt=listenersToRemove.length;i<cnt;i++) {
          delete this.app._listeners[listenersToRemove[i]];
        }
  ...
  (function(){
      ...
      init: function(app_function) {
        ...
        if (this.debug) {
          this.bindToAllEvents(function(e, data) {
            this.app.log(this.app.toString(), e.cleaned_type, data || {});
          });
        }
      }
    });
    ...
    // Alias for route('get', ...)
    Sammy.Application.prototype.get = _routeWrapper('get');

    // Alias for route('post', ...)
    Sammy.Application.prototype.post = _routeWrapper('post');

    // Alias for route('put', ...)
    Sammy.Application.prototype.put = _routeWrapper('put');

    // Alias for route('delete', ...)
    Sammy.Application.prototype.del = _routeWrapper('delete');

    // Alias for route('any', ...)
    Sammy.Application.prototype.any = _routeWrapper('any');
    ...
    //
    Sammy.Application.prototype.before = function(options, callback) {
    ...
    Sammy.Application.prototype.bindToAllEvents = function(callback) {
      ...
      //@TODO move to component. Listen only to this. component's events
      // App can bind an ALL events from ALL components
      $.each(this._listeners.keys(true), function(i, name) {
        if (app.APP_EVENTS.indexOf(name) == -1) {
          app.bind(name.replace(app.eventNamespace(), ''), callback);
        }
      });
      return this;
    };
    ...
    Sammy.Application.prototype._parseQueryString = function(path) {
    ...
    Sammy.Application.prototype._parseParamPair = function(params, key, value) {
      if (params[key]) {
  ...
  (function() {
    ...
    Sammy.EventContext = Sammy.Object.extend({
      ...
      //   the route.
      //
      init: function(app, verb, path, params, target) {
        this.app    = app;
        this.verb   = verb;
        this.path   = path;
