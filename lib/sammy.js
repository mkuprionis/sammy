...
(function($, window) {
  ...
  (function(){
    ...
    // this is called manually in render() of components using a shared drawing space
    // (i.e. sip tree vs user list vs customer list)
    // and marks the other elements as inactive - event handlers should respect if(IsActive)
    Sammy.Component.prototype.Activate = function() {
      var $this = this;
      
      var activator = function(ix, comp) {
        if(!/^UI:/.test(comp.name) && comp.$element == $this.$element) {
          comp.IsActive = comp.name == $this.name;
        }
        $.each(comp.components, activator);
      };
      
      $.each(this.app.components, activator);
    };
...
//    }
  ...
  (function() {
    ...
    Sammy.EventContext.prototype.composeAsync = function(content) {
    	...
    	args.shift();
    	var passArgs = [], comps = args;
    	for(var i = 0; i < comps.length; ++i) {
    		var c = comps[i], component, renderer;
    		if($.isArray(c)) {
    			component = c[0];
    			renderer = c[1];
    		} else {
    	...
    	passArgs.push(function(rc) {
			...
			content = comps[0].composeContent.apply($this, composeArgs);
			rc.swap(content);
    	});
